...
public class WorldGenerator {
	...
	public void generateLevel(Level level, final int randomizations) {
		...
		for (int i = 0; i < numMountains; i++)
			addMountain(rand.nextInt(WIDTH), rand.nextInt(100) + 20, rand.nextInt(100) + 20, rand.nextInt(75) + 25);
		addCaves();
		level.tiles = tiles;
	}
	...
	private void addMountain(int x, int w, int h, int percentStone) {
		for (int yy = 0; yy <= 2 * h; yy++)
			for (int xx = -w; xx <= w; xx++)
				try {
					if (xx * xx + yy * yy <= w * h)
						tiles[x + xx + yy * WIDTH] = rand.nextInt(100) <= percentStone ? Tile.stoneTile : Tile.dirtTile;
				} catch (ArrayIndexOutOfBoundsException e) {
					continue;
				}
